...
export const SessionReadReceiptTable = sqliteTable(
  "session_read_receipt",
  {
    session_id: text()
      .$type<SessionSchema.ID>()
      .notNull()
    ...
    digest: text().notNull(),
    settled_seq: integer().notNull(),
    call_id: text(),
    ...Timestamps,
  },
  (table) => [primaryKey({ columns: [table.session_id, table.canonical_path] })],
)
