...
export const getUsage = (input: { model: Provider.Model; usage: Usage; metadata?: ProviderMetadata }) => {
  ...
  const piUsage = isRecord(input.metadata?.["pi"]?.["usage"]) ? input.metadata?.["pi"]?.["usage"] : undefined
  const piCost = isRecord(piUsage?.cost) ? piUsage.cost.total : undefined
  return {
    cost:
      typeof totalNanoAiu === "number" && Number.isFinite(totalNanoAiu) && totalNanoAiu >= 0
        ? new Decimal(totalNanoAiu).div(100_000_000_000).toNumber()
        : typeof piCost === "number" && Number.isFinite(piCost) && piCost >= 0
          ? piCost
          : safe(
              new Decimal(0)
